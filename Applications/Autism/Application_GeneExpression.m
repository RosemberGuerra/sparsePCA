...
% The data should be loaded manualy from Autism.csv file %
load('AutismDataset.mat')
X = Autism';
X([12,13,27],:) = [];% Removing mislabel individuals %

X = normalize(X);
ColNaN = sum(isnan(X))~=0;
X = X(:,~ColNaN);
X = normalize(X); %
...
upb = 0.76; %max(vecnorm(X))^2; This value was obtained manualy to get 
