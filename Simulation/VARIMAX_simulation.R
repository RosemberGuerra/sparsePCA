...
Simulation = foreach(i=1:Ndatasets,
        ...
        .combine=rbind)%dopar%{
          ...
          PZ1 = PCAvarimax(X = Out1$X,k = Out1$k , ps = Out1$Propsparse )   # P sparse
          PZ2 = PCAvarimax(X = Out2$X,k = Out2$k , ps = Out2$Propsparse )   # P and W sparse
          PZ3 = PCAvarimax(X = Out3$X,k = Out3$k , ps = Out3$Propsparse )   # W sparse
...
## Results in a table ##
Methodology = rep('Varimax',Ndatasets)
...
DataVarimax = cbind(Methodology,p_sparse2, MatrixInfo,
                 ErrorLW1,ErrorScores1,MR01, Svar1,
                 ErrorLW2,ErrorScores2,MR02, Svar2,
                 Correlation3,CorrelationZ3,MR03, Svar3)

write.table(DataVarimax, file = "JointDataVarimax.txt", sep = ",",row.names = FALSE)
save.image(file = "VarimaxResults.RData")
